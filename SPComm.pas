...
//  2017-11-07      - 修改成支持XE10.2，修改PChar为PAnsiChar
...
type
    ...
    protected
      ...
      function HandleWriteData( lpOverlappedWrite: POverlapped;
                                pDataToWrite: PAnsiChar; dwNumberOfBytesToWrite: DWORD): Boolean;        //PCHar 20190119
    public
      hCommFile:          THandle;
      hCloseEvent:        THandle;
      hComm32Window:      THandle;
...
//      procedure SetParity( p : TParity );
    ...
    protected
      ...
      procedure ReceiveData(Buffer: PAnsiChar; BufferLength: Word);   //PChar 20190119
    ...
    public
      ...
      procedure StartComm;
      procedure StopComm;
      function WriteCommData( pDataToWrite: PAnsiChar; dwSizeofDataToWrite: Word ): Boolean;  //PChar 20190119
...
begin
     ...
     if Length(FCommName) < 5 then
        ...
        FCommName := '\\.\' + FCommName;  //支持打开COM10及以上       //2017.07.01  steedsky    20190119
...
//
//
function TComm.WriteCommData( pDataToWrite: PAnsiChar; dwSizeofDataToWrite: Word ): Boolean;  //PChar-->PAnsiCHar 20190119
var
   Buffer: Pointer;
begin
...
procedure TComm.ReceiveData(Buffer: PAnsiChar; BufferLength: Word);      //PChar->PAnsiChar 20190119
begin
...
begin
  with msg do
     case msg of
          PWM_GOTCOMMDATA:
          begin
               ReceiveData( PAnsiChar(LParam), WParam );  //PChar->PAnsiChar 2019.0119
               LocalFree( LParam )
          end;
...
begin
     ...
     if not SetupReadEvent( @overlappedRead,
                          //  @szInputBuffer, INPUTBUFFERSIZE,            //当前行替换为下一行 20190119
...
//                                            @szInputBuffer,     //当前行替换为下一行   20190119
     ...
     begin
          ...
          begin
               ...
               if not HandleWriteData( @overlappedWrite,
                                       PAnsiChar(msg.lParam), DWORD(msg.wParam) ) then   //PChar->PAnsiChar 20190119
               begin
...
function TWriteThread.HandleWriteData( lpOverlappedWrite: POverlapped;
         pDataToWrite: PAnsiChar; dwNumberOfBytesToWrite: DWORD): Boolean;     //PChar->PAnsiChar 20190119
var
   dwLastError,

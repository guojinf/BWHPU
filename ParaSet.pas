...
begin
  ...
  With ParaInfo do
  begin
    ParaInfo.PumpNum:=StrToInt(Pump_Num_ComboBox.Text); //泵数量
    //ParaInfo.PLCType:=StrToInt         //文本中设置    PLC类型
    ParaInfo.PumpOff_ATSoftColse_EN:=PumpCheckBox1.Checked; //关软件是否关油泵，1关
    //ParaInfo.ParaDispEN[i]:=        //文本中设置  是否显示参数
    //ParaInfo.ParaDispAdd[i]:=       //文本中设置      在PLC中的地址

    //Comm232para
    ParaInfo.CommType:=CommTypeRadioGroup1.ItemIndex;   //哪种通讯方式
   ...
   // ParaInfo.NetPara.ProtocolType := StrToInt(          //在文本中设置 通讯协议
    if ParaInfo.commType=COMMTYPE_COM then
    begin
      ParaInfo.ComPara.Enable:=True;
    end else
